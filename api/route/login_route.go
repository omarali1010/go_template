...
func NewLoginRouter(app *bootstrap.Application, timeout time.Duration, router chi.Router) {
	userRepo := repository.NewUserRepository(app.DB)

	loginUC := usecase.NewLoginUsecase(userRepo, app.JWTService, app.JWTService, timeout)

	sc := &controller.LoginController{
		LoginUsecase: loginUC,
	}

	router.Post("/public/login", sc.Login)
}
